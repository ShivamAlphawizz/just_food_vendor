...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<Null> getSallerDetail() async {
    ...
    if (_isNetworkAvail) {
      ...
      var parameter = {Id: CUR_USERID};
      print("seller detail api========================== ${getSellerDetails}");
      print("${parameter}");
      apiBaseHelper.postAPICall(getSellerDetails, parameter).then(
            (getdata) async {
          bool error = getdata["error"];
          ...
          if (!error) {
            ...
            var status = data[STATUS];
            var storeLogo = data[StoreLogo];
            print("ooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo ${data['open_close_status']}");
            setState(() {
              isSwitched = data["open_close_status"]=="1"?true:false;
            });
            print("bank name : $bankName");
            saveUserDetail(
              id!,
  ...
  Future updateSellerStatus() async{
    print("switch value here ${isSwitched}");
    ...
    if (response.statusCode == 200) {
      ...
      print("kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk ${jsonResponse}");
...
//========================= Third Row Implimentation ===========================
    ...
    if (true) {
      if (_isNetworkAvail) {
        try {
          ...
          List<String> ids = [];
          print("model data here ${model.itemList.length}");
          for(int i=0;i<model!.itemList!.length;i++){
            print("sds ${tempList[0].itemList}");
            print("sds======================================== ${tempList[0].itemList![i]}");
            print("sds ${tempList[0].itemList![i].id}");
            ids.add(tempList[0].itemList![i].id.toString());
          }
          if (item) {
            parameter[ORDERITEMID] = ids.join(",");
        ...
        if (scrollLoadmore) {
          ...
          /* if (activeStatus != null) {
            ...
            parameter[ActiveStatus] = activeStatus!;
              ...
              print("get order api paras ${getOrdersApi} and ${parameter}");
          apiBaseHelper.postAPICall(getOrdersApi, parameter).then(
                (getdata) async {
              bool error = getdata["error"];
              String? msg = getdata["message"];
              ...
              if (!error) {
                ...
                if (data.length != 0) {
                  ...
                  orderList.addAll(tempList);
                  print("sssjjhshkbs ${orderList.length}");
                  scrollLoadmore = true;
                  scrollOffset = scrollOffset + perPage;
                } else {
                  scrollLoadmore = false;
  ...
  //       await post(getUpdateApi, body: parameter, headers: headers)
  //           .timeout(Duration(seconds: timeOut));
  //         print("api here ${getUpdateApi}");
  //         print("api params here ${parameter}");
  //       var getdata = json.decode(response.body);
  //       bool error = getdata["error"];
  //       String? msg = getdata["message"];
  //
